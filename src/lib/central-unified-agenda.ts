...
function taskHref(
  ...
  const saleId = nullable(row.sale_id);
  const orderId = nullable(row.purchase_order_id);
  const contactId = nullable(row.central_contact_id);

  if (scope === "supplements") {
    if (saleId) return `/vendas/${saleId}`;
    if (orderId) return `/pedidos-fornecedor/${orderId}`;
...
export async function getCentralUnifiedAgendaSnapshot({
  ...
  if (canFitness) {
    ...
    for (const raw of postSaleResult.data ?? []) {
      ...
      items.push({
        ...
        operation_scope: "fitness",
        source_type: "sale_post_sale",
        source_id: text(row.id),
        editable_task_id: null,
        category: "post_sale",
        title: `Pós-venda · ${text(
          row.customer_name,
          "Cliente",
        )}`,
        subtitle:
        ...
        status: normalizeStatus(row.status),
        priority:
          normalizeStatus(row.status) === "planned" &&
          dueDate < today
            ? "urgent"
            : "normal",
        contact_name: nullable(row.customer_name),
    ...
    for (const raw of ordersResult.data ?? []) {
      ...
      items.push({
        ...
        editable_task_id: null,
        category: "supplier",
        title: `Chegada prevista · ${text(
          row.supplier_name,
          "Fornecedor",
        )}`,
        subtitle:
          nullable(row.product_summary) ??
          `${Number(row.pending_units ?? 0)} unidade(s) a caminho`,
        due_at: dateAtNoon(dueDate),
        due_date: dueDate,
        status: "planned",
        priority:
  ...
  const unique = [
    ...new Map(
      items.map((item) => [item.event_key, item]),
    ).values(),
  ];
