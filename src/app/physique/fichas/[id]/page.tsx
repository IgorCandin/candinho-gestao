    ...
    <section className="physique-page">
      <header className="physique-subpage-header">
        ...
        <div>
          ...
          <h1>{plan.title}</h1>
          <p>
            {athlete ? <Link href={`/physique/atletas/${athlete.id}`}>{athlete.display_name}</Link> : "Atleta"}
      ...
      {attachmentPending && (
        <article className="physique-nexus-preview-head">
          <Paperclip size={18} />
          <div>
            <strong>Ficha salva; PDF original pendente</strong>
            <span>
              A estrutura do treino foi preservada. O envio do PDF falhou depois da criação da ficha; anexe o arquivo novamente na seção abaixo.
            </span>
          </div>
        </article>
      )}
      ...
      {plan.ai_imported_at && (
        <article className="physique-nexus-preview-head">
          ...
          <div>
            ...
            <span>O PDF original foi preservado quando o upload foi concluído e a estrutura abaixo foi salva para consulta rápida.</span>
          </div>
        </article>
      )}
      ...
      <article className="physique-panel">
        ...
        {days.length === 0 ? (
          <div className="physique-empty compact"><Dumbbell size={23} /><strong>Nenhum dia estruturado nesta ficha</strong></div>
        ) : (
                    ...
                    {dayExercises.map((exercise) => (
                      <article key={exercise.id}>
                        ...
                        <div>
                          ...
                          <span>{[exercise.sets_text, exercise.reps_text].filter(Boolean).join(" × ") || "Séries/repetições não informadas"}</span>
                          {(exercise.technique || exercise.load_guidance || exercise.notes) && (
                            <small>{[exercise.technique, exercise.load_guidance, exercise.notes].filter(Boolean).join(" · ")}</small>
                          )}
                        </div>
      ...
      <article className="physique-panel">
        <div className="physique-panel-title">
          <div><span>Armazenamento privado</span><h2>Anexos da ficha</h2></div>
          <b>{attachments.length}</b>
        </div>
        <PhysiqueTrainingAttachmentUpload planId={plan.id} />
        {attachments.length === 0 ? (
          <div className="physique-empty compact"><Paperclip size={23} /><strong>Nenhum anexo nesta ficha</strong></div>
        ) : (
            ...
            {attachments.map((attachment) => (
              <div key={attachment.id}>
                ...
                {attachment.signed_url && (
                  <a href={attachment.signed_url} target="_blank" rel="noreferrer"><Download size={15} /> Abrir</a>
                )}
              </div>
            ))}
          </div>
