...
import { AlertTriangle, BarChart3, MessageSquareText, PackageSearch, Radar, ShoppingBag, UserRoundSearch } from "lucide-react";
...
export default async function SupplementsHomePage(){
  const access=await getCurrentUserAccess(); if(access.role==="sales") redirect("/produtos");
  ...
  const {data:postSale}=await supabase.from("post_sale_batch_summary").select("*").maybeSingle();
  return <>
    <DemoBanner/>
    <section className="operation-home-hero operation-home-no-heading">
      ...
      <div className="operation-home-kpis">
        ...
        <Link href="/pos-venda"><span>Pós-venda</span><strong>{Number(postSale?.open_count??0)}</strong><small>{Number(postSale?.overdue_count??0)} vencidos · {Number(postSale?.today_count??0)} hoje</small></Link>
    ...
    <section className="operation-home-actions">
      <Link href="/pos-venda"><MessageSquareText size={20}/><div><strong>Pós-venda com Nexus</strong><span>Acompanhamentos consolidados e mensagem gerada por IA</span></div></Link>
      <Link href="/produtos"><PackageSearch size={20}/><div><strong>Produtos e catálogo</strong><span>Consultar produtos e gerar PDF para o cliente</span></div></Link>
      <Link href="/clientes/radar"><Radar size={20}/><div><strong>Radar</strong><span>Recompra, reativação e oportunidades</span></div></Link>
      <Link href="/leads"><UserRoundSearch size={20}/><div><strong>Leads</strong><span>Retomar oportunidades que ainda não viraram venda</span></div></Link>
