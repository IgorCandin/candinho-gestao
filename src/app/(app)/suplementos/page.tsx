...
import { NexusTodaySnapshot } from "@/components/nexus-today-snapshot";
...
export default async function SupplementsHomePage() {
  ...
  const canRefreshNexus =
    access.role === "admin" || access.canWriteSupplements;

  const [data, investment, radar, nexus] = await Promise.all([
    getDashboard(),
    getOperationInvestmentSnapshot(),
    getCustomerOpportunityRadarSummary(),
    getNexusBrief({ refresh: canRefreshNexus, signalLimit: 35 }),
  ]);
  ...
  const postSaleNow =
    Number(postSale?.today_count ?? 0) +
    Number(postSale?.overdue_count ?? 0);
  ...
  const nexusActionCount =
    nexus.counts.urgent + nexus.counts.attention + nexus.counts.opportunity;
      ...
      <div className="operation-home-v2">
        <header className="operation-today-head">
          <div>
            ...
            <p>
              O Nexus organiza a primeira decisão; os módulos continuam sendo
              a fonte de verdade para executar e registrar o trabalho.
            </p>
          </div>
          <div className="operation-today-summary">
            <span>Sinais para revisar</span>
            <strong>{nexusActionCount || urgentCount}</strong>
            <small>Nexus + agenda operacional</small>
          </div>
        </header>

        <NexusTodaySnapshot brief={nexus} />
        ...
        <section className="operation-kpi-grid-v2">
          ...
          <Link href="/pedidos-pendentes">
            ...
            <small>
              {data.pendingPaymentCount} a receber · {data.pendingDeliveryCount}{" "}
              a entregar
            </small>
          </Link>
          <Link href="/pos-venda">
            ...
            <small>
              {Number(postSale?.overdue_count ?? 0)} atrasado(s) ·{" "}
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              <h2>Faça primeiro · Agenda</h2>
              <p>
                Depois da triagem do Nexus, esta é a lista formal de tarefas
                vencidas ou marcadas para hoje.
              </p>
            </div>
            <Link className="button ghost compact-button" href="/agenda">
              Abrir agenda
            </Link>
          </div>
          ...
          {todayActions.length === 0 ? (
            <div className="empty compact">
              ...
              Use o Nexus ou os cards acima para escolher a próxima ação.
            </div>
          ) : (
            <div className="operation-priority-list-v2">
              {todayActions.map((item) => (
                <Link
                  className="operation-priority-item-v2"
        ...
        <section className="operation-quick-grid-v2">
          <Link href="/vendas/nova">
            ...
            <div>
              ...
              <span>Registrar interesse concreto e seguir até a conversão</span>
            </div>
          </Link>
          <Link href="/leads/novo">
            ...
            <div>
              ...
              <span>Registrar quem apenas perguntou ou demonstrou interesse</span>
            </div>
          </Link>
          <Link href="/suplementos/nexus">
            ...
            <div>
              ...
              <span>Conversar com a operação inteira e entender a próxima decisão</span>
            </div>
          </Link>
          <Link href="/clientes">
            <MessageSquareText size={20} />
            <div>
              <strong>CRM e relacionamento</strong>
              <span>Clientes, retornos, histórico, vínculos e pós-venda</span>
            </div>
          </Link>
          <Link href="/clientes/radar">
            ...
            <div>
              ...
              <span>Recompra, reativação e clientes prováveis</span>
            </div>
          </Link>
          <Link href="/estoque">
            <Boxes size={20} />
            <div>
              ...
              <span>Saldo, conferência, movimentações e reposição</span>
            </div>
          </Link>
          <Link href="/pedidos-fornecedor/planejamento">
            ...
            <div>
              ...
              <span>Giro, cobertura e produtos que precisam de reposição</span>
            </div>
          </Link>
          <Link href="/produtos">
            ...
            <div>
              ...
              <span>Cadastro, consulta, sabores e materiais comerciais</span>
            </div>
          </Link>
          <Link href="/parceiros">
            <Handshake size={20} />
            <div>
              ...
              <span>Rede, pontos de estoque, acessos e recompensas</span>
            </div>
          </Link>
          <Link href="/suplementos/painel">
            ...
            <div>
              ...
              <span>Faturamento, lucro e indicadores sem competir com a rotina</span>
            </div>
          </Link>
        </section>
        ...
        {operationalAttention > 0 && (
          <div className="operation-alert-compact">
            ...
            <div>
              <strong>
                {operationalAttention} ponto(s) operacional(is) precisam de atenção
              </strong>
              <span>
                {data.operational.overdue_payment_count} cobrança(s) vencida(s) ·{" "}
                {data.operational.supplier_orders_open_count} pedido(s) de fornecedor ·{" "}
                {data.operational.out_of_stock_products} ruptura(s)
              </span>
            </div>
            <Link className="button ghost compact-button" href="/estoque">
              Revisar operação
            </Link>
          </div>
        )}
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              ...
              <p>
                Indicadores continuam disponíveis, mas ficam abaixo do trabalho
                que precisa ser feito hoje.
              </p>
            </div>
            <Link className="button ghost" href="/suplementos/painel">
              <BarChart3 size={16} /> Painel Gerencial
            </Link>
          </div>
          <div className="panel-body">
            <p className="form-help">
              Possíveis clientes no Radar: <strong>{radar.possible_customers}</strong>{" "}
              · Alta prioridade: <strong>{radar.high_priority}</strong> · A receber:{" "}
              <strong>{data.operational.pending_payment_count}</strong> venda(s).
            </p>
          </div>
        </article>
