...
import { AlertTriangle, BarChart3, PackageSearch, Radar, ShoppingBag, UserRoundSearch } from "lucide-react";
...
export default async function SupplementsHomePage(){
  ...
  return <>
    ...
    <section className="operation-home-hero operation-home-no-heading">
      <Link className="operation-home-primary" href="/vendas/nova">
        <ShoppingBag size={24}/>
        <div>
      ...
      <div className="operation-home-kpis">
        <Link href="/pedidos-pendentes">
          ...
          <small>{data.pendingDeliveryCount} entregar · {data.pendingPaymentCount} receber</small>
        </Link>
    ...
    {(data.operational.overdue_payment_count>0||data.operational.out_of_stock_products>0)&&
      <article className="operation-home-alert">
        <AlertTriangle size={18}/>
        <div>
          <strong>Atenção hoje</strong>
          <span>{data.operational.overdue_payment_count} pagamento(s) vencido(s) · {data.operational.out_of_stock_products} produto(s) zerado(s)</span>
        </div>
      </article>
    }
  </>;
