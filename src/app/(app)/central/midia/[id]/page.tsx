...
import { getCentralContacts, getCentralIntegrationReadiness, getCentralMediaAssetDetails } from "@/lib/central-data";
...
export default async function CentralMediaDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [asset, contacts, readiness] = await Promise.all([
  ...
  return <>
    <div className="central-media-detail-back">
      <Link href="/central/midia" className="button ghost"><ArrowLeft size={15}/>Voltar à biblioteca</Link>
    </div>

    <PageHeader
      eyebrow="Candinho Central · Mídia"
      title={asset.original_filename ?? "Detalhe do arquivo"}
      description={asset.description_ai ?? "Arquivo armazenado na biblioteca privada da Candinho Company."}
    />
    ...
    <section className="central-media-detail-layout">
      <article className="panel central-media-detail-preview-panel">
        <div className="central-media-detail-preview">
          {isImage && asset.signed_url
            ? <img src={asset.signed_url} alt={asset.description_ai ?? asset.original_filename ?? "Mídia"}/>
            : isVideo && asset.signed_url
              ? <video src={asset.signed_url} controls preload="metadata"/>
      ...
      <div className="central-media-detail-side">
        <article className="panel">
          <div className="panel-heading"><span><Link2 size={16}/><strong>Contexto</strong></span></div>
          <div className="panel-body">
            <CentralMediaLinkForm
              assetId={asset.id}
              currentContactId={asset.contact_id}
              currentConversationId={asset.conversation_id}
              contacts={contacts}
            />
          </div>
        </article>
        ...
        <article className="panel">
          <div className="panel-heading"><span><Sparkles size={16}/><strong>Nexus Mídia</strong></span></div>
    ...
    <article className="panel central-media-detail-info">
      <div className="panel-heading"><span><FileText size={16}/><strong>Informações do arquivo</strong></span></div>
      <div className="central-media-detail-info-grid">
        ...
        <div><span>Histórico antigo</span><strong>{asset.conversation_id ? "Vínculo de conversa preservado" : "Sem vínculo de conversa"}</strong></div>
      </div>
      {asset.tags.length > 0 && <div className="central-media-detail-tags">{asset.tags.map((tag) => <i key={tag}>{tag}</i>)}</div>}
    </article>
  </>;
}
