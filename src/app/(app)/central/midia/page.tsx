...
import { CheckCircle2, Clock3, FileText, ImageIcon, Link2, Search, Sparkles, Video } from "lucide-react";
import { CentralMediaUploader } from "@/components/central-media-uploader";
import { PageHeader } from "@/components/page-header";
import { getCentralContacts, getCentralIntegrationReadiness, getCentralMediaAssets } from "@/lib/central-data";
...
export default async function CentralMediaPage({ searchParams }: { searchParams: Promise<{ q?: string; scope?: string; kind?: string; ai?: string; contact?: string }> }) {
  ...
  const [contacts, readiness] = await Promise.all([getCentralContacts(), access.canManageUsers ? getCentralIntegrationReadiness() : Promise.resolve(null)]);
  const requestedContact = params.contact && contacts.some((item) => item.id === params.contact) ? params.contact : null;
  const assets = await getCentralMediaAssets(params.q ?? "", requestedScope, requestedKind, requestedAi, requestedContact);
  ...
  const linkedCount = assets.filter((asset) => Boolean(asset.contact_id || asset.conversation_id)).length;

  return <>
    <PageHeader eyebrow="Candinho Central" title="Biblioteca de mídia" description="Organize fotos, vídeos e documentos por operação, cliente e contexto de atendimento." />
    <article className="panel central-media-upload-panel"><div className="panel-body"><CentralMediaUploader scopes={allowedScopes} contacts={contacts}/></div></article>
    ...
    <section className="central-media-summary central-media-summary-v2">
      ...
      <div><span>Ligados a contexto</span><strong>{linkedCount}</strong></div>
    ...
    <form className="central-media-search central-media-search-v2" method="get">
      ...
      <button className="button ghost" type="submit">Filtrar</button>
    </form>

    {assets.length === 0 ? <article className="panel"><div className="empty"><ImageIcon size={28}/><strong>Nenhuma mídia encontrada</strong>Envie o primeiro arquivo ou altere os filtros.</div></article> : <section className="central-media-grid">{assets.map((asset) => {
      ...
      return <Link href={`/central/midia/${asset.id}`} className="central-media-card" key={asset.id}>
        <div className="central-media-preview">{image ? <img src={asset.signed_url!} alt={asset.description_ai ?? asset.original_filename ?? "Mídia"}/> : video ? <video src={asset.signed_url!} muted preload="metadata"/> : <AssetIcon mime={asset.mime_type}/>}</div>
        <div className="central-media-card-body">
          ...
          {asset.contact_name && <small className="central-media-linked"><Link2 size={12}/>{asset.contact_name}</small>}
