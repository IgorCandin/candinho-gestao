...
function parseMoney(
  ...
  if (
    ...
    throw new Error(
      "Uma das faturas possui um valor inválido.",
    );
  }
...
export async function markBankInvoicePaid(
  formData: FormData,
) {
...
export async function saveBankInvoices(
  ...
  if (!uuidPattern.test(cardId)) {
    throw new Error(
      "Cartão inválido para atualização.",
    );
  }

  if (
    referenceMonths.length !== 12 ||
    referenceMonths.some(
      (month) =>
        !monthPattern.test(month),
    )
  ) {
    throw new Error(
      "A atualização precisa conter exatamente os próximos 12 meses.",
    );
  }
  ...
  if (
    mode === "todas" &&
    uuidPattern.test(nextCardId)
  ) {
    redirect(
      `/bank/faturas?acao=atualizar&modo=todas&cartao=${encodeURIComponent(
        nextCardId,
      )}&salvo=1`,
    );
  }
...
export async function createBankCard(
  ...
  redirect(
    "/bank/faturas?salvo=cartao-criado",
  );
}
