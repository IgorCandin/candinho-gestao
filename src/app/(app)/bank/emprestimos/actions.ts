...
function parseMoney(value: FormDataEntryValue | null, options?: { required?: boolean; label?: string }) {
  ...
  if (!Number.isFinite(parsed) || parsed <= 0) {
    throw new Error(`Informe um ${label} válido maior que zero.`);
  }
...
export async function createBankDebt(formData: FormData) {
  const name = String(formData.get("name") ?? "").trim();
  const debtType = String(formData.get("debt_type") ?? "loan");
  const creditorName = String(formData.get("creditor_name") ?? "").trim() || null;
  const originalAmount = parseMoney(formData.get("original_amount"), { label: "valor total" });
  const monthlyAmount = parseMoney(formData.get("monthly_amount"), { required: false, label: "valor da parcela" });
  const startDateRaw = String(formData.get("start_date") ?? "").trim();
  const nextDueDateRaw = String(formData.get("next_due_date") ?? "").trim();
  ...
  redirect("/bank/emprestimos?salvo=criada");
}

export async function payBankDebtInstallment(formData: FormData) {
  const debtId = String(formData.get("debt_id") ?? "");
  const amount = parseMoney(formData.get("amount"), { required: false, label: "valor do pagamento" });
  ...
  const { error } = await supabase.rpc("bank_pay_debt_installment", {
    p_debt_id: debtId,
    p_amount: amount,
    p_paid_on: paidOn,
  ...
  revalidateBankDebtPaths();
  redirect("/bank/emprestimos?salvo=paga");
}

export async function postponeBankDebtPayment(formData: FormData) {
  const debtId = String(formData.get("debt_id") ?? "");
  const notes = String(formData.get("notes") ?? "").trim() || null;
  ...
  revalidateBankDebtPaths();
  redirect("/bank/emprestimos?salvo=adiada");
}
