...
export default async function LeadDetailsPage({params}:{params:Promise<{id:string}>}) {
  ...
  return <>
    <DemoBanner/>
    <PageHeader eyebrow="Lead" title={lead.customer_name} description="Informações do contato comercial, sabor de interesse e orçamento vinculado." action={action}/>
    ...
    <section className="lead-details-layout">
      <div className="lead-details-main">
        ...
        <article className="panel">
          <div className="panel-head"><div><h2>Produtos de interesse</h2><p>O sabor pode permanecer em aberto no lead e ser definido quando virar orçamento.</p></div><strong>{lead.items.length}</strong></div>
          <div className="panel-body lead-product-list">
            {lead.items.length>0
              ?lead.items.map((item)=>{
                const flavor=flavorByItem.get(item.id);
                return <div className="lead-product-card" key={item.id}>
                  <div className="sale-item-image">{item.product_image_url?<img src={item.product_image_url} alt={item.product_name}/>:<ImageIcon size={30}/>}</div>
                  <div>
