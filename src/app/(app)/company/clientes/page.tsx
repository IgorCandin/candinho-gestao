...
export default async function CompanyCustomersPage() {
  ...
  for (const row of fitness.filter((item) => !linkedFitnessIds.has(item.id))) customers.push({ id: row.id, fitnessId: row.id, name: row.name, phone: row.phone, city: row.city, operations: row.total_purchases > 0 ? ["Fitness"] : [], purchaseCount: row.total_purchases, totalSpent: row.total_spent, lastPurchaseOn: row.last_purchase_on, activityCount: row.total_purchases, activitySummary: row.total_purchases ? `${row.total_purchases} venda(s) em Fitness` : "Cadastro sem movimentações", detailHref: `/company/clientes/fitness/${row.id}`, editHref: `/company/clientes/fitness/${row.id}/editar` });
  const deduplicated = new Map<string, CompanyCustomerRow>();
  for (const customer of customers.sort((a, b) => b.activityCount - a.activityCount)) {
    const digits = customer.phone?.replace(/\D/g, "") ?? "";
    const key = digits.length >= 8 ? `phone:${digits}` : `id:${customer.id}`;
    if (!deduplicated.has(key)) deduplicated.set(key, customer);
  }
  const visibleCustomers = [...deduplicated.values()].sort((a, b) => a.name.localeCompare(b.name, "pt-BR"));
  return <CompanyCustomerRegistry customers={visibleCustomers} canWriteSupplements={access.canWriteSupplements} canWriteFitness={access.canWriteFitness}/>;
}
