  ...
  return (
    <>
      ...
      <PageHeader
        ...
        description={`${order.item_count} ${order.item_count === 1 ? "produto" : "produtos"} · destino ${order.destination_code}`}
      ...
      <section className="supplier-order-detail-layout">
        <div className="supplier-order-detail-main">
          <article className="panel">
            <div className="panel-head"><div><h2>Itens do pedido</h2><p>Cada item pode ser recebido separadamente ou em partes.</p></div><Boxes size={20} /></div>
              ...
              {order.items.map((item) => (
                <article className="supplier-detail-item" key={item.id}>
                  <div className="supplier-detail-item-top">
                    ...
                    <span className={`date-status ${item.item_status === "received" ? "green" : "orange"}`}>{statusLabel(item.item_status)}</span>
                  </div>

                  {item.waiting_sales.length > 0 && (
                    <div className="waiting-sales-box">
                      <div className="waiting-sales-head"><UserRound size={17} /><div><strong>Vendas aguardando este produto</strong><span>O recebimento reservará automaticamente as unidades disponíveis.</span></div></div>
                  ...
                  {item.quantity_pending > 0 ? <ReceivePurchaseItemForm item={item} /> : <div className="supplier-item-complete"><PackageCheck size={18} /><span>Item totalmente recebido</span></div>}
                </article>
              ))}
            </div>
          </article>
