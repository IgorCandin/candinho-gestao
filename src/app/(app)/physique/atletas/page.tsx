    ...
    <section className="physique-page">
      <header className="physique-subpage-header">
        ...
        <div>
          ...
          <h1>Atletas</h1>
          <p>Base preparada para conectar o atleta ao restante do ecossistema Candinho.</p>
        </div>
      </header>
      ...
      {athletes.length === 0 ? (
        <div className="physique-empty">
          ...
          <p>A operação ainda não foi inicializada. O cadastro será liberado em uma etapa futura.</p>
        </div>
      ) : (
        <div className="physique-athlete-list">
          {athletes.map((athlete) => (
            <Link href={`/physique/atletas/${athlete.id}`} key={athlete.id}>
              <div>
                <small>{athlete.status}</small>
                <strong>{athlete.display_name}</strong>
                <span>{athlete.primary_goal ?? "Objetivo ainda não informado"}</span>
              </div>
              <div className="physique-athlete-plan-count">
                <Dumbbell size={15} />
                {athlete.training_plan_count} ficha(s)
              </div>
            </Link>
          ))}
        </div>
