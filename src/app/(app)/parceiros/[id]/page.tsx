...
import { Boxes, CircleDollarSign, Edit3, Gift, Handshake, History, MapPin, Phone, Store, TrendingUp, UsersRound } from "lucide-react";
...
export default async function PartnerDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const progressPct =
    partner.reward_type === "gift_per_sales" && (partner.target_sales ?? 0) > 0
      ? Math.round((partner.progress_sales / (partner.target_sales ?? 1)) * 100)
  ...
  return <>
    ...
    <section className="partner-detail-layout">
      <div className="partner-detail-main">
            ...
            {partner.reward_type === "gift_per_sales" ? <>
              ...
              {partner.reward_units_due > 0 && (
                <p className="partner-reward-alert">
                  <Gift size={17} />
                  {partner.reward_units_due} meta(s) de brinde alcançada(s) neste histórico. Registre os acertos já entregues para iniciar um novo ciclo.
                </p>
              )}
            </> : (
              <div className="partner-progress-large">
                ...
                <div>
                  <strong>{partner.reward_type === "fixed_per_sale" || partner.reward_type === "percentage" ? formatCurrency(partner.estimated_reward_amount) : "Manual"}</strong>
                ...
                {legacyHistory.map((movement) => (
                  <div className="bank-charge-item" key={movement.id}>
                    <div className="bank-charge-date">
                      <strong>{formatDateOnly(movement.occurredAt)}</strong>
                      <span>{movement.movementType}</span>
                    </div>
                    <div className="bank-charge-main">
                      <strong>{movement.quantity}x {movement.product}</strong>
                      <span>{movement.destinationCode ?? movement.originCode ?? "Parceiro"}</span>
                    </div>
        ...
        <article className="panel">
          <div className="panel-head">
            <div><h2>Vendas vinculadas</h2><p>Histórico comercial atribuído a este parceiro.</p></div>
