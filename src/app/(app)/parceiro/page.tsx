...
export default async function PartnerPortalPage() {
  ...
  return <>
    ...
    <section className="partner-portal-hero">
      <article className="partner-portal-contract panel"><div className="panel-body"><span className="partner-portal-icon"><Handshake size={26}/></span><div><small>Condição da parceria</small><strong>{profile.partnership_percent.toLocaleString("pt-BR", { maximumFractionDigits: 2 })}%</strong><p>{profile.settlement_rule ?? profile.reward_description ?? "Regra de parceria cadastrada pela Candinho."}</p></div></div></article>
      <article className="partner-portal-contract panel"><div className="panel-body"><span className="partner-portal-icon"><Gift size={26}/></span><div><small>Benefício / recompensa</small><strong>{profile.reward_description ?? profile.reward_type ?? "Conforme parceria"}</strong><p>{profile.settlement_frequency ? `Acerto: ${profile.settlement_frequency}` : "Acompanhe os ciclos junto à Candinho."}</p></div></div></article>
    </section>
    ...
    <article className="panel partner-monthly-panel"><div className="panel-head"><div><h2>Histórico mensal</h2><p>Vendas e faturamento atribuídos ao seu perfil nos últimos 12 meses.</p></div></div><div className="table-wrap"><table className="partner-portal-table"><thead><tr><th>Mês</th><th>Vendas</th><th>Unidades</th><th>Faturamento</th><th>% parceria</th><th>Referência estimada</th></tr></thead><tbody>{history.map((month) => <tr key={month.month_start}><td>{formatMonthYear(month.month_start)}</td><td>{month.sales_count}</td><td>{month.units_sold}</td><td>{formatCurrency(month.gross_sales)}</td><td>{month.partnership_percent.toLocaleString("pt-BR", { maximumFractionDigits: 2 })}%</td><td>{formatCurrency(month.estimated_partner_share)}</td></tr>)}</tbody></table></div></article>
  </>;
}
