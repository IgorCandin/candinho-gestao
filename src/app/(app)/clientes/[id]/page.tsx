  ...
  return (
    <>
      ...
      <PageHeader
        eyebrow="Cliente"
        title={customer.name}
        description="Ficha comercial, cuidados de atendimento, contatos, pÃ³s-venda e histÃ³rico completo."
        action={(
      ...
      <div className="customer-radar-strip">
        ...
        <div>
          {customer.next_followup_at && <span>PrÃ³ximo retorno: <strong>{formatDateOnly(customer.next_followup_at)}</strong></span>}
          {customer.last_contact_at && <span>Ãšltimo contato: <strong>{formatDate(customer.last_contact_at)}</strong></span>}
        </div>
      </div>
      ...
      <section className="customer-profile-grid">
        <article className="panel">
          <div className="panel-head"><div><h2>Dados do cliente</h2><p>InformaÃ§Ãµes disponÃ­veis</p></div><UserRound size={19} /></div>
          <div className="panel-body sale-detail-list">
            ...
            <Line label="ReferÃªncia" value={customer.reference} />
            <Line label="Etiquetas" value={customer.tags} />
            <Line label="Ãšltima compra" value={customer.last_purchase_at ? formatDate(customer.last_purchase_at) : null} />
            <Line label="Ãšltimo resultado" value={customer.last_contact_outcome} />
            <Line label="ObservaÃ§Ãµes" value={customer.notes} />
          </div>
        ...
        {care ? (
          <article className="panel customer-care-panel">
            ...
            <div className="panel-body sale-detail-list">
              {customer.sensitive_to_caffeine && <Line label="CafeÃ­na" value="SensÃ­vel Ã  cafeÃ­na" />}
              {customer.anxiety_or_insomnia && <Line label="Ansiedade/sono" value="Possui ansiedade ou insÃ´nia" />}
              <Line label="Produtos que nÃ£o devem ser indicados" value={customer.prohibited_products} />
              <Line label="PreferÃªncia de abordagem" value={customer.approach_preferences} />
            </div>
          </article>
        ) : (
          <article className="panel customer-care-empty">
            <div className="panel-head"><div><h2>Cuidados no atendimento</h2><p>Nenhuma restriÃ§Ã£o registrada</p></div><AlertTriangle size={19} /></div>
            <div className="empty compact"><strong>Ficha livre</strong>Use â€œEditar dadosâ€ para registrar sensibilidades, restriÃ§Ãµes e preferÃªncias.</div>
          </article>
        )}
      </section>
      ...
      {pending.length > 0 && (
        <article className="panel customer-history-panel">
          ...
          <div className="table-wrap"><table><thead><tr><th>Produto</th><th>Data</th><th>Pagamento</th><th>Entrega</th><th>Total</th><th /></tr></thead><tbody>{pending.map((order: (typeof pending)[number]) => <tr key={order.id}><td>{order.product_summary ?? "â€”"}</td><td>{formatDateOnly(order.business_date)}</td><td><Badge value={order.payment_status} /></td><td><Badge value={order.delivery_status} /></td><td className="amount">{formatCurrency(order.total_amount)}</td><td><Link className="button ghost compact-button" href={`/pedidos-pendentes/${order.id}`}>Abrir</Link></td></tr>)}</tbody></table></div>
        </article>
      )}
