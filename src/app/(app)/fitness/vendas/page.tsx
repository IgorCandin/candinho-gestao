...
function customerLabel(name: string, city: string | null) {
  return city?.trim() ? `${name} - ${city}` : name;
}
  ...
  return (
    <>
      <PageHeader
        ...
        title="Vendas"
        description="Histórico de vendas, pagamentos, entregas e reservas de estoque."
        action={
              ...
              {sales.map((sale) => (
                <tr key={sale.id}>
                  <td>
                    ...
                    >
                      {customerLabel(sale.customer_name, sale.city)}
                    </Link>
                  </td>
                  ...
                  <td><Badge value={sale.reservation_status} /></td>
