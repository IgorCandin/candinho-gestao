...
export default async function ProductDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [product, swipe, flavorSummaryResult, flavorInventoryResult, pendingResult] = await Promise.all([
    getProductDetails(id),
    getEntitySwipeNavigation("product", id),
    supabase.from("product_flavor_summary").select("*").eq("product_id", id).maybeSingle(),
    supabase
      .from("product_flavor_inventory_overview")
      .select("flavor_id,flavor_name,active,display_order,physical_quantity,reserved_quantity,available_quantity,incoming_quantity")
      .eq("product_id", id)
      .eq("active", true)
      .order("display_order")
      .order("flavor_name"),
  ...
  const flavorRows = [...flavorMap.values()].sort((a, b) => a.order - b.order || a.name.localeCompare(b.name, "pt-BR"));
    ...
    <>
      ...
      <PageHeader
        ...
        action={
          <div className="page-header-action-group">
            <Link className="button gold" href={`/produtos/${product.id}/editar`}><Edit3 size={16} />Editar produto</Link>
            <Link className="button ghost" href={`/estoque/${product.id}`}><Warehouse size={16} />Ver estoque</Link>
            <Link className="button ghost" href="/produtos"><ArrowLeft size={16} />Voltar</Link>
          </div>
        }
      />
        ...
        <article className="panel">
          <div className="panel-head">
            <div>
              ...
              <p>O estoque total continua sendo o saldo oficial do produto. Abaixo está a distribuição operacional desse total.</p>
            </div>
            {historyPending > 0 && (
              <Link className="button ghost compact-button" href={`/produtos/sabores/historico?produto=${product.id}`}>
          ...
          <div className="table-wrap">
            <table>
              <thead>
                <tr><th>Sabor</th><th>Físico</th><th>Reservado</th><th>Disponível</th><th>A caminho</th></tr>
              </thead>
              <tbody>
                ...
                {flavorRows.length === 0 && <tr><td colSpan={5}>Nenhum sabor ativo encontrado.</td></tr>}
              </tbody>
            </table>
          </div>
        </article>
      ...
      <section className="product-details-layout">
        ...
        <div className="product-details-side">
          ...
          {(product.description || product.objective || product.ideal_profile || product.information || product.quick_message) && (
            <article className="panel">
