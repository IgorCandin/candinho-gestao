...
export default async function ProductsPage() {
  ...
  return <>
    ...
    <PageHeader
      ...
      action={<ProductCatalogActions canWrite={access.canWriteSupplements} products={products} />}
    />
    ...
    {access.role === "sales" && (
      <article className="panel sales-location-panel">
        <div className="panel-head">
          <div><h2>Disponibilidade por filial / parceiro</h2><p>Onde existe estoque livre ou reposição prevista.</p></div>
        </div>
        <div className="table-wrap">
          <table className="table">
            <thead><tr><th>Produto</th><th>Local</th><th>Disponível</th><th>A caminho</th></tr></thead>
            <tbody>
              {locations
                .filter((row)=>row.available_quantity>0||row.incoming_quantity>0)
                .map((row)=>
                  <tr key={`${row.product_id}-${row.location_id}`}>
                    <td><strong>{row.product_name}</strong></td>
                    <td>{row.location_name}<small className="crm-cell-note">{row.location_code}</small></td>
