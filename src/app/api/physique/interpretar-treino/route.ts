  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (response.ok) {
      ...
      try {
        ...
        throw new Error("O Nexus retornou uma ficha em formato inválido.");
      }
  ...
  try {
    ...
    if (!access.canManageUsers && access.role !== "admin") {
      return NextResponse.json(
        { error: "Sem permissão para interpretar fichas da Physique." },
        { status: 403 },
      );
    }
    ...
    if (file.type !== "application/pdf" && !file.name.toLowerCase().endsWith(".pdf")) {
      return NextResponse.json(
        { error: "A importação automática aceita PDF nesta etapa." },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Não foi possível interpretar a ficha." },
      { status: 500 },
    );
  }
}
