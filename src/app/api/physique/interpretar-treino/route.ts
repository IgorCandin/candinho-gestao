...
const OUTPUT_SCHEMA = {
  type: "object",
  properties: {
    title: { type: "string" }, goal: { type: ["string", "null"] }, coach_name: { type: ["string", "null"] },
    starts_on: { type: ["string", "null"] }, ends_on: { type: ["string", "null"] }, notes: { type: ["string", "null"] },
    days: { type: "array", items: { type: "object", properties: {
      day_label: { type: "string" }, focus: { type: ["string", "null"] }, notes: { type: ["string", "null"] },
      exercises: { type: "array", items: { type: "object", properties: {
        exercise_name: { type: "string" }, sets_text: { type: ["string", "null"] }, reps_text: { type: ["string", "null"] },
        rest_seconds: { type: ["integer", "null"] }, technique: { type: ["string", "null"] }, load_guidance: { type: ["string", "null"] }, notes: { type: ["string", "null"] },
      }, required: ["exercise_name", "sets_text", "reps_text", "rest_seconds", "technique", "load_guidance", "notes"], additionalProperties: false } },
    }, required: ["day_label", "focus", "notes", "exercises"], additionalProperties: false } },
    summary: { type: "string" },
  },
...
function normalizeDate(value: unknown) { return typeof value === "string" && datePattern.test(value) ? value : null; }
function normalizeTraining(data: JsonRecord) { return { ...data, starts_on: normalizeDate(data.starts_on), ends_on: normalizeDate(data.ends_on) }; }
async function responseJson(response: Response): Promise<JsonRecord> { try { return (await response.json()) as JsonRecord; } catch { return {}; } }
...
async function callOpenAI(apiKey: string, file: File) {
  ...
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    if (response.ok) {
      ...
      catch { throw new Error("O Nexus retornou uma ficha em formato inválido."); }
      return { data: normalizeTraining(parsed), model };
    }
...
export async function POST(request: Request) {
  try {
    ...
    if (!access.canManageUsers && access.role !== "admin") return NextResponse.json({ error: "Sem permissão para interpretar fichas da Physique." }, { status: 403 });
    ...
    if (file.type !== "application/pdf" && !file.name.toLowerCase().endsWith(".pdf")) return NextResponse.json({ error: "A importação automática aceita PDF nesta etapa." }, { status: 400 });
