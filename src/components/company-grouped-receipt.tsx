...
const orderKey = (row: CompletionOrder) => `${row.operation ?? "Suplementos"}:${row.id}`;
const today = () => new Intl.DateTimeFormat("en-CA", { timeZone: "America/Sao_Paulo", year: "numeric", month: "2-digit", day: "2-digit" }).format(new Date());

export function CompanyGroupedReceipt({ orders, onClose }: { orders: CompletionOrder[]; onClose: () => void }) {
  const router = useRouter();
  const [selected, setSelected] = useState<string[]>(orders.map(orderKey));
  ...
  const chosen = useMemo(() => orders.filter((row) => selected.includes(orderKey(row))), [orders, selected]);
  const mixedOperations = chosen.some((row) => row.operation === "Fitness");
  const hasSupplement = chosen.some((row) => row.operation !== "Fitness");
  ...
  const validMode = mixedOperations ? difference < .005 : difference < .005 || (paid < total ? mode === "partial" || mode === "discount" : mode === "interest");

  async function save(event: React.FormEvent) {
    event.preventDefault();
    if (chosen.length < 2) { setError("Selecione pelo menos duas vendas."); return; }
    if (mixedOperations && !hasSupplement) { setError("Selecione também uma venda de Suplementos para este pagamento conjunto."); return; }
    if (!Number.isFinite(paid) || paid <= 0) { setError("Informe o valor recebido."); return; }
    if (!validMode) { setError(mixedOperations ? "Para pagar Fitness e Suplementos juntos, informe o total exato. Ajustes e pagamentos parciais continuam disponíveis em cada venda." : "Escolha como tratar a diferença antes de salvar."); return; }
    ...
    try {
      const { error: rpcError } = mixedOperations
        ? await createClient().rpc("register_company_cross_operation_receipt_v1", {
            p_batch_id: batchId,
            p_supplement_sale_ids: chosen.filter((row) => row.operation !== "Fitness").map((row) => row.id),
            p_fitness_sale_ids: chosen.filter((row) => row.operation === "Fitness").map((row) => row.id),
            p_amount: paid, p_received_on: date, p_payment_method: method, p_notes: notes.trim() || null,
          })
        : await createClient().rpc("register_company_grouped_receipt_v2", {
  ...
  return <div className="company-completion-backdrop company-grouped-receipt-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section className="company-completion-dialog company-grouped-receipt" role="dialog" aria-modal="true" aria-labelledby="grouped-receipt-title">
      ...
      <form onSubmit={save} className="company-grouped-receipt-form">
        <fieldset><legend>Vendas incluídas</legend>{orders.map((row) => <label key={orderKey(row)} className="company-grouped-receipt-sale"><input type="checkbox" checked={selected.includes(orderKey(row))} onChange={(event) => { setSelected(event.target.checked ? [...selected, orderKey(row)] : selected.filter((id) => id !== orderKey(row))); setMode("exact"); }}/><span><strong>{row.product_summary || "Venda"}</strong><small>{row.operation ?? "Suplementos"} · {row.location_name}</small></span><b>{formatCurrency(outstanding(row))}</b></label>)}</fieldset>
        ...
        {mixedOperations && <p className="form-help">Fitness e Suplementos: este pagamento conjunto exige o total exato. Para desconto, juros ou pagamento parcial, ajuste as vendas individualmente.</p>}
        ...
        {!mixedOperations && mode === "partial" && difference > .005 && <label>Quando o cliente vai pagar o restante?<input className="input" type="date" min={date} required value={remainingDueOn} onChange={(event) => setRemainingDueOn(event.target.value)}/></label>}
        {!mixedOperations && (mode === "discount" || mode === "interest") && difference > .005 && <p className="form-help">O ajuste só é permitido em vendas sem recebimentos anteriores e sem parcelas. Entregas já realizadas continuam preservadas.</p>}
        ...
        <footer><button className="button ghost" type="button" onClick={onClose}>Cancelar</button><button className="button gold" type="submit" disabled={working || chosen.length < 2 || !validMode || (mixedOperations && !hasSupplement)}>{working ? "Salvando..." : "Confirmar pagamento conjunto"}</button></footer>
      </form>
    </section>
  </div>;
}
