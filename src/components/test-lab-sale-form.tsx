...
export function TestLabSaleForm({ operation, customers, products }: { operation: TestLabOperation; customers: TestLabCustomer[]; products: TestLabStockRow[] }) {
  ...
  const product=useMemo(()=>products.find((p: (typeof products)[number])=>p.product_id===productId),[products,productId]);
  ...
  return <form className="panel-body test-lab-form" onSubmit={submit}>
    <div className="form-grid-two">
      <label className="field"><span>Cliente fictício</span><select className="select" required value={customerId} onChange={(e)=>setCustomerId(e.target.value)}>{customers.map((c: (typeof customers)[number])=><option key={c.id} value={c.id}>{c.name}</option>)}</select></label>
      <label className="field"><span>Produto fictício</span><select className="select" required value={productId} onChange={(e)=>setProductId(e.target.value)}>{products.map((p: (typeof products)[number])=><option key={p.product_id} value={p.product_id}>{p.name}{p.variant_label?` · ${p.variant_label}`:""} · disp. {p.available_quantity}</option>)}</select></label>
