...
export function CompanyCareWorkspace({ items }: { items: CompanyCareItem[] }) {
  ...
  const counts = useMemo(() => {
    const people = (match: (item: CompanyCareItem) => boolean) => new Set(items.filter(match).map((item) => item.customerId)).size;
    return { today: people((item) => Boolean(item.dueOn && item.dueOn <= today)), waiting: people((item) => item.kind === "waiting"), post_sale: people((item) => item.kind === "post_sale"), follow_up: people((item) => item.kind === "follow_up"), all: people(() => true) };
  }, [items, today]);
  ...
  const grouped = useMemo(() => {
    const people = new Map<string, CompanyCareItem[]>();
    for (const item of visible) {
      const group = people.get(item.customerId);
      if (group) group.push(item); else people.set(item.customerId, [item]);
    }
    return Array.from(people.values());
  }, [visible]);
  const filters: Array<{ id: Filter; label: string; count: number }> = [{ id: "today", label: "Atender hoje", count: counts.today }, { id: "waiting", label: "Aguardando resposta", count: counts.waiting }, { id: "post_sale", label: "Pós-venda", count: counts.post_sale }, { id: "follow_up", label: "Retornos", count: counts.follow_up }, { id: "all", label: "Todos", count: counts.all }];
  ...
  return <div className="company-workspace-v2 company-care-v2">
    ...
    <section className="company-workspace-panel"><div className="company-workspace-toolbar"><div>{filters.map((item) => <button key={item.id} className={filter === item.id ? "active" : ""} onClick={() => setFilter(item.id)}>{item.label} · {item.count}</button>)}</div><label><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cliente, cidade ou motivo"/></label></div>{message ? <p className="company-care-feedback">{message}</p> : null}<p className="company-workspace-count">{grouped.length} pessoa(s) · {visible.length} assunto(s) nesta fila</p>
      <div className="company-care-grid">{grouped.map((personItems) => { const person = personItems[0]; const wa = whatsapp(person.phone, person.customerName); return <article className="company-care-person" key={person.customerId}><div className="company-care-person-head"><div className="company-care-avatar"><UserRound/></div><div><h2>{person.customerName}</h2><small>{[person.city, person.phone].filter(Boolean).join(" · ")}</small></div>{wa ? <a href={wa}><MessageCircle size={15}/>WhatsApp</a> : null}</div><p>{personItems.length} assunto(s) neste contato</p>{personItems.map((item) => { const overdue = Boolean(item.dueOn && item.dueOn < today); const open = expanded === item.id; const context = contexts[item.customerId]; return <div className={`company-care-card ${open ? "is-open" : ""}`} key={item.id}>
        <div className="company-care-copy"><div><span className={overdue ? "overdue" : ""}>{overdue ? "Atrasado" : item.dueOn === today ? "Hoje" : item.kind === "waiting" ? "Aguardando" : "Agendado"}</span><small>{item.operation}</small></div><strong>{item.title}</strong><p>{item.note}</p><small>{item.dueOn ? `Próxima ação: ${formatDateOnly(item.dueOn)}` : ""}</small></div>
        <div className="company-care-actions"><button type="button" onClick={() => void openCard(item)}>{open ? "Fechar" : "Resolver este assunto"}</button></div>
