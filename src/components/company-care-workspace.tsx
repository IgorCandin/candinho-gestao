...
export function CompanyCareWorkspace({ items }: { items: CompanyCareItem[] }) {
  ...
  return <div className="company-workspace-v2 company-care-v2">
    ...
    <section className="company-workspace-panel"><div className="company-workspace-toolbar"><div>{filters.map((item) => <button key={item.id} className={filter === item.id ? "active" : ""} onClick={() => setFilter(item.id)}>{item.label} · {item.count}</button>)}</div><label><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Cliente, cidade ou motivo"/></label></div>{message ? <p className="company-care-feedback">{message}</p> : null}<p className="company-workspace-count">{visible.length} pessoa(s) nesta fila</p>
      <div className="company-care-grid">{visible.map((item) => { const wa = whatsapp(item.phone, item.customerName); const overdue = Boolean(item.dueOn && item.dueOn < today); const open = expanded === item.id; return <article className={`company-care-card ${open ? "is-open" : ""}`} key={item.id}><div className="company-care-avatar"><UserRound/></div><div className="company-care-copy"><div><span className={overdue ? "overdue" : ""}>{overdue ? "Atrasado" : item.dueOn === today ? "Hoje" : item.kind === "waiting" ? "Aguardando" : "Agendado"}</span><small>{item.operation}</small></div><h2>{item.customerName}</h2><strong>{item.title}</strong><p>{item.note}</p><small>{[item.city, item.phone, item.dueOn ? `Próxima ação: ${formatDateOnly(item.dueOn)}` : null].filter(Boolean).join(" · ")}</small></div><div className="company-care-actions">{wa ? <a href={wa} target="_blank" rel="noreferrer"><MessageCircle size={15}/> WhatsApp</a> : null}<button type="button" onClick={() => setExpanded(open ? null : item.id)}>{open ? "Fechar" : "Resolver atendimento"}</button></div>{open ? <div className="company-care-inline"><div><strong>Finalizar sem sair desta tela</strong><span>Registre o resultado da conversa e defina se esse cliente deve voltar para a fila.</span></div><div className="company-care-inline-actions"><button type="button" disabled={busy === item.id} onClick={() => void resolve(item, false)}>{busy === item.id ? <LoaderCircle className="spin" size={15}/> : <CheckCircle2 size={15}/>}Finalizado</button><button type="button" disabled={busy === item.id} onClick={() => void resolve(item, true)}><CalendarClock size={15}/>Preferiu esperar 1 mês</button><Link href={`/company/clientes/${item.customerId}`}>Ver ficha completa</Link></div></div> : null}</article>; })}</div>
