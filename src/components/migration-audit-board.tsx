...
export function MigrationAuditBoard({ rows, saved }: { rows: MigrationAuditRow[]; saved: SavedAudit[] }) {
  ...
  async function save(row: MigrationAuditRow, state: AuditState) {
    ...
    if (error) setValues((current) => { const nextValues = new Map(current); if (existing) nextValues.set(row.key, existing); else nextValues.delete(row.key); return nextValues; });
    setSaving(null);
  }
