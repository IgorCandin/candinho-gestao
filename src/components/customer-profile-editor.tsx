  ...
  if (!open) {
    return (
      <div className="customer-profile-editor-trigger">
        <button className="button ghost" type="button" onClick={() => setOpen(true)}><Pencil size={16} />Editar dados</button>
        {message && <span className="crm-inline-message"><Check size={14} />{message}</span>}
      </div>
    );
  }
  ...
  return (
    <form className="customer-profile-editor" onSubmit={submit}>
      <div className="customer-editor-head">
        <div><strong>Editar ficha do cliente</strong><span>Dados, restrições e preferências de atendimento.</span></div>
        <button className="icon-button" type="button" aria-label="Fechar edição" onClick={() => setOpen(false)}><X size={17} /></button>
      </div>
      <div className="form-grid-two">
        ...
        <label className="switch-row field-span-two"><div><strong>Cliente ativo</strong><span>Clientes inativos continuam no histórico, mas não aparecem nos novos formulários.</span></div><input type="checkbox" checked={form.active} onChange={(event) => setField("active", event.target.checked)} /></label>
      </div>
      <div className="customer-editor-footer">
        <button className="button ghost" type="button" onClick={() => setOpen(false)}>Cancelar</button>
        <button className="button gold" disabled={loading}>{loading ? <LoaderCircle className="spin" size={17} /> : <Save size={17} />}{loading ? "Salvando" : "Salvar alterações"}</button>
      </div>
