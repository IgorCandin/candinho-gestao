...
export function CompanyPurchaseSuggestions({ suggestions }: { suggestions: CompanyPurchaseSuggestion[] }) {
  ...
  return <section className="company-orders-section company-purchase-suggestions">
    <div className="company-section-heading"><div><span>Nexus · Reposição</span><h2>Lista prática de compra</h2><p>Suplementos considera produtos equivalentes. Fitness confere a grade por categoria e sugere somente os tamanhos totalmente ausentes.</p></div><div className="company-purchase-selection"><ShoppingCart size={17}/><strong>{selectedRows.length}</strong><span>item(ns) · {selectedRows.reduce((sum, item) => sum + item.quantity, 0)} un.</span></div></div>
    {suggestions.length ? <>
