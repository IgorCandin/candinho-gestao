...
function labelHtml(label: ProductLabel, copies: number) {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"/><title>Etiqueta - ${escapeHtml(label.name)}</title><style>@page{size:80mm 101.5mm;margin:0}*{box-sizing:border-box}html,body{margin:0;background:#fff;color:#000;font-family:Arial,sans-serif}.label{width:80mm;height:98mm;padding:3mm 3.5mm;display:grid;grid-template-rows:13mm minmax(21mm,1fr) 16mm 24mm 12mm;gap:1mm;break-inside:avoid;page-break-inside:avoid;page-break-after:always;background:#fff}.label>*{min-height:0;border:.35mm solid #000;border-radius:4mm;break-inside:avoid;page-break-inside:avoid}.label-head{padding:2mm;display:flex;align-items:center;justify-content:center}.label-head img{display:block;width:46mm;max-height:9mm;object-fit:contain}.product{padding:3mm}.product h1{margin:1mm 0 0;font-size:13pt;line-height:1.05;overflow-wrap:anywhere;word-break:break-word}.variation{margin:1.2mm 0 0;font-size:8.5pt;font-weight:800}.product>span,.prices span,.barcode>span{font-size:6.5pt;font-weight:900;letter-spacing:.5px}.prices{display:grid;grid-template-columns:repeat(2,minmax(0,1fr))}.prices div{display:grid;align-content:center;gap:.6mm;padding:2.5mm}.prices div+div{border-left:.35mm solid #000}.prices b{font-size:13pt;line-height:1.05;white-space:nowrap}.barcode{padding:2mm;text-align:center;display:grid;justify-items:center;align-content:start;gap:.5mm}.barcode svg{display:block;width:52mm;height:18mm}.codes{display:flex;width:100%;justify-content:space-between;gap:2mm}.codes strong{font-size:6.5pt;letter-spacing:.3px;white-space:nowrap}footer{padding:1.5mm;display:grid;gap:.4mm;text-align:center;align-content:center}footer strong{font-size:8pt}footer span{font-size:6.2pt;line-height:1.1}@media screen{body{padding:14px;background:#eee}.label{margin:0 auto 14px;box-shadow:0 1px 6px #777}}</style></head><body>${Array.from({ length: Math.max(1, Math.min(100, copies)) }, () => page).join("")}<script>window.addEventListener('load',()=>window.print())<\/script></body></html>`;
}
