...
function labelHtml(label: ProductLabel, copies: number) {
  ...
  return `<!doctype html><html lang="pt-BR"><head><meta charset="utf-8"/><title>Etiqueta - ${escapeHtml(label.name)}</title><style>@page{size:80mm 101.5mm;margin:0}*{box-sizing:border-box}html,body{margin:0;background:#fff;color:#000;font-family:Arial,sans-serif}.label{width:80mm;height:101.5mm;padding:3mm 3.5mm;display:flex;flex-direction:column;gap:1.3mm;page-break-after:always;background:#fff}.label>*{border:.35mm solid #000;border-radius:4mm}.label-head{height:14mm;padding:2mm;display:flex;align-items:center;justify-content:center}.label-head img{display:block;width:46mm;max-height:10mm;object-fit:contain}.product{min-height:22mm;padding:3mm}.product h1{margin:1mm 0 0;font-size:13pt;line-height:1.05;overflow-wrap:anywhere;word-break:break-word}.variation{margin:1.2mm 0 0;font-size:8.5pt;font-weight:800}.product>span,.prices span,.barcode>span{font-size:6.5pt;font-weight:900;letter-spacing:.5px}.prices{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));min-height:17mm}.prices div{display:grid;align-content:center;gap:.6mm;padding:2.5mm}.prices div+div{border-left:.35mm solid #000}.prices b{font-size:13pt;line-height:1.05;white-space:nowrap}.barcode{min-height:24mm;padding:2mm;text-align:center;display:grid;justify-items:center;align-content:start;gap:.5mm}.barcode svg{display:block;width:52mm;height:19mm}.codes{display:flex;width:100%;justify-content:space-between;gap:2mm}.codes strong{font-size:6.5pt;letter-spacing:.3px;white-space:nowrap}footer{min-height:13mm;padding:2mm;display:grid;gap:.5mm;text-align:center;align-content:center}footer strong{font-size:8.5pt}footer span{font-size:6.5pt;line-height:1.15}@media screen{body{padding:14px;background:#eee}.label{margin:0 auto 14px;box-shadow:0 1px 6px #777}}</style></head><body>${Array.from({ length: Math.max(1, Math.min(100, copies)) }, () => page).join("")}<script>window.addEventListener('load',()=>window.print())<\/script></body></html>`;
}
