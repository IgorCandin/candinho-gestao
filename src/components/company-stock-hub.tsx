...
import { ArrowRightLeft, ClipboardCheck, PackageMinus, PackageSearch, RefreshCcw, ScanLine, Settings2, Sparkles } from "lucide-react";
import { useMemo, useState } from "react";
import { ProductSalesCategoryIntelligence, type ProductSalesCategoryIntelligenceRow } from "@/components/product-sales-category-intelligence";

type FitnessItem = { id: string; name: string; size: string; color: string; physical: number; available: number };

export function CompanyStockHub({ fitness, categoryRows, canUpdateCategories, initialOperation = "supplements" }: { fitness: FitnessItem[]; categoryRows: ProductSalesCategoryIntelligenceRow[]; canUpdateCategories: boolean; initialOperation?: "supplements" | "fitness" }) {
  ...
  const cards = operation === "supplements" ? [
    ...
    ["Estoque", "Peças, tamanhos, cores e disponibilidade dentro da Company.", "/company/produtos?operacao=Fitness", PackageSearch],
    ...
    ["Despesa / baixa", "Avaria ou uso interno sem gerar uma venda.", "/company/estoque/despesa?operacao=fitness", PackageMinus],
  ] as const;
  return <div className="company-workspace-v2 company-stock-hub">
    <header className="company-workspace-head"><div><span>COMPANY · ESTOQUE</span><h1>Qual operação você quer movimentar?</h1><p>Contagem, correção e reconciliação respeitam a operação escolhida. Nada de abrir uma rotina de Suplementos para acertar uma peça Fitness.</p></div><Link className="button company-blue" href={`/company/estoque/despesa?operacao=${operation}`}><PackageMinus size={16}/>Despesa / baixa</Link></header>
    ...
    {operation === "supplements" ? <section id="categorias-nexus"><ProductSalesCategoryIntelligence rows={categoryRows} canUpdate={canUpdateCategories}/></section> : null}
