...
import { ArrowRightLeft, ClipboardCheck, PackageMinus, PackageSearch, RefreshCcw, ScanLine, Settings2 } from "lucide-react";
...
export function CompanyStockHub({ fitness, initialOperation = "supplements" }: { fitness: FitnessItem[]; initialOperation?: "supplements" | "fitness" }) {
  ...
  return <div className="company-workspace-v2 company-stock-hub">
    <header className="company-workspace-head"><div><span>COMPANY · ESTOQUE</span><h1>Qual operação você quer movimentar?</h1><p>Contagem, correção e reconciliação respeitam a operação escolhida. Nada de abrir uma rotina de Suplementos para acertar uma peça Fitness.</p></div><Link className="button company-blue" href="/company/estoque/despesa"><PackageMinus size={16}/>Despesa / baixa</Link></header>
