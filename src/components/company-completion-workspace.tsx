...
export function CompanyCompletionWorkspace({ orders, itemMedia }: { orders: CompletionOrder[]; itemMedia: Record<string, SaleItemMedia[]> }) {
  ...
  const [groupCustomerKey, setGroupCustomerKey] = useState<string | null>(null);
  ...
  const payableByCustomer = useMemo(() => {
    ...
    for (const order of orders) {
      if (!order.customer_key || amount(order) <= .005) continue;
      groups.set(order.customer_key, [...(groups.get(order.customer_key) ?? []), order]);
    }
    return groups;
  }, [orders]);
  const canPayTogether = (key: string | undefined) => {
    const group = key ? payableByCustomer.get(key) ?? [] : [];
    return group.length > 1 && group.some((order) => order.operation !== "Fitness");
  };
  ...
  return <div className="company-workspace-v2">
    ...
    {groupCustomerKey && <CompanyGroupedReceipt key={groupCustomerKey} orders={payableByCustomer.get(groupCustomerKey) ?? []} onClose={() => setGroupCustomerKey(null)}/>}
  </div>;
}
