...
export function SaleDetailsView({
  ...
  backLabel: string;
  eyebrow?: string;
  flavorByItem?: Record<string, string>;
}) {
  return <>
    <DemoBanner/>
    <PageHeader
      ...
      description="Confira produtos, sabores, estoque, pagamento, entrega e informações internas da venda."
      action={
        <div className="page-header-actions">
          {sale.quote_id&&<a className="button gold" href={`/api/orcamentos/${sale.quote_id}/pdf`} target="_blank" rel="noreferrer"><FileText size={16}/>Abrir PDF</a>}
          <Link className="button ghost" href={backHref}><ArrowLeft size={16}/>{backLabel}</Link>
        </div>
      }
    />
    ...
    <section className="sale-details-layout">
      <div className="sale-details-main">
        <article className="panel">
          ...
          <div className="panel-body sale-items-list">
            {sale.items.map((item)=>{
              const flavor = flavorByItem[item.id];
              return (
                <div className="sale-item-card detailed" key={item.id}>
                  ...
                  <div className="sale-item-copy">
                    ...
                    {reservationLabel(item.reservation_status,item.quantity_reserved,item.quantity_requested)&&(
                      <small className={`reservation-copy ${item.reservation_status}`}>
                        {reservationLabel(item.reservation_status,item.quantity_reserved,item.quantity_requested)}
                      </small>
                    )}
                  </div>
                  ...
                  <div className="sale-item-numbers">
                    ...
                    <small>Custo interno: {formatCurrency(item.unit_cost)}</small>
                    <small>{item.price_condition}</small>
                  </div>
                </div>
              );
            })}
        ...
        {sale.gift_product_name&&sale.gift_quantity>0&&(
          <article className="panel">
            <div className="panel-head"><div><h2>Brinde</h2><p>Produto incluído sem valor de venda e já considerado no custo/lucro.</p></div><Gift size={19}/></div>
            <div className="panel-body sale-detail-list">
              <DetailLine label="Produto" value={sale.gift_product_name}/>
              <DetailLine label="Quantidade" value={`${sale.gift_quantity} un.`}/>
            </div>
          </article>
        )}

        <article className="panel">
          <div className="panel-head"><div><h2>Atualizar venda</h2><p>Corrija o cliente sem cancelar a venda ou atualize pagamento e entrega.</p></div></div>
          <div className="panel-body sale-update-stack">
            <ChangeSaleCustomer saleId={sale.id} currentCustomerId={sale.customer_id} currentCustomerName={sale.customer_name}/>
            <SaleStatusActions saleId={sale.id} generalStatus={sale.general_status} paymentStatus={sale.payment_status} deliveryStatus={sale.delivery_status}/>
          </div>
        </article>
      </div>
      ...
      <aside className="sale-details-side">
        ...
        {sale.partner_name&&(
          <article className="panel">
            <div className="panel-head"><div><h2>Parceria</h2><p>Venda contabilizada para o parceiro</p></div><Handshake size={19}/></div>
            <div className="panel-body sale-detail-list"><DetailLine label="Parceiro" value={sale.partner_name}/></div>
          </article>
        )}
        ...
        {sale.discount_amount>0&&(
          <article className="panel">
            <div className="panel-body sale-detail-list">
              <DetailLine label="Subtotal" value={formatCurrency(sale.gross_amount)}/>
              <DetailLine label="Desconto" value={`- ${formatCurrency(sale.discount_amount)}`}/>
            </div>
          </article>
        )}
        ...
        <article className="panel sale-total-panel">
          ...
          <div><span>Total da venda{sale.quote_number?` · Orçamento #${sale.quote_number}`:""}</span><strong>{formatCurrency(sale.total_amount)}</strong></div>
        </article>
      </aside>
    </section>
  </>;
