...
export function InventoryActions({ products, locations, locationRows, initialProductId = "" }: {
  ...
  useEffect(() => {
    ...
    async function loadFlavors() {
      ...
      setFlavorInventory((stockResult.data ?? []).map((row) => ({
  ...
  const current = useMemo(
    () => locationRows.find((row) => row.product_id === productId && row.location_id === locationId) ?? null,
  ...
  const selectedFlavor = productFlavors.find((flavor) => flavor.id === flavorId) ?? null;
  const selectedFlavorInventory = flavorInventory.find((row) => row.flavorId === flavorId && row.locationId === locationId) ?? null;
  ...
  async function submit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (productFlavors.length > 0 && !flavorId) throw new Error("Selecione o sabor.");
      ...
      <form className="inventory-modal" onSubmit={submit}>
        <div className="inventory-modal-head">
          <div>
            ...
            <h2>{mode === "count" ? "Contagem física" : mode === "adjust" ? "Ajuste manual" : "Transferência"}</h2>
            <p>
              {mode === "count"
                ? "Informe a quantidade realmente contada. Produto com sabores é contado sabor por sabor."
                : mode === "adjust"
                  ? "Use valor positivo para entrada e negativo para saída. Produto com sabores exige o sabor."
                  : "Move apenas unidades disponíveis do sabor selecionado, sem mexer nas reservas."}
            </p>
          </div>
          <button className="icon-button" type="button" aria-label="Fechar" onClick={close}><X size={18}/></button>
        </div>
        ...
        <div className="inventory-modal-body">
          <label className="field">
            ...
            <select
              ...
              required
              value={productId}
              onChange={(event) => { setProductId(event.target.value); setFlavorId(""); }}
          ...
          {productFlavors.length > 0 && (
            <label className="field">
              <span>Sabor</span>
              <select className="select" required value={flavorId} onChange={(event) => setFlavorId(event.target.value)}>
                <option value="">Selecione o sabor</option>
                {productFlavors.map((flavor) => {
                  const row = flavorInventory.find((entry) => entry.flavorId === flavor.id && entry.locationId === locationId);
                  return <option key={flavor.id} value={flavor.id}>{flavor.name} · disp. {row?.available ?? 0}</option>;
                })}
              </select>
            </label>
          )}
          ...
          <label className="field">
            ...
            <select className="select" required value={locationId} onChange={(event) => setLocationId(event.target.value)}>
          ...
          {mode === "transfer" && (
            <label className="field">
              ...
              <select className="select" required value={destinationId} onChange={(event) => setDestinationId(event.target.value)}>
                <option value="">Selecione o destino</option>
                {locations.filter((location) => location.id !== locationId).map((location) => <option key={location.id} value={location.id}>{location.code} · {location.name}</option>)}
              </select>
            </label>
          )}

          <label className="field">
            <span>{mode === "count" ? "Quantidade contada" : mode === "adjust" ? "Variação do saldo" : "Quantidade a transferir"}</span>
            <input
              className="input"
              type="number"
              required
          ...
          {selectedProduct && (
            <div className="inventory-current-strip">
              <span>{selectedProduct.product_name}{selectedFlavor ? ` · ${selectedFlavor.name}` : ""}</span>
              <span>Físico <strong>{selectedFlavorInventory?.physical ?? current?.physical_quantity ?? 0}</strong></span>
              <span>Reservado <strong>{selectedFlavorInventory?.reserved ?? current?.reserved_quantity ?? 0}</strong></span>
              <span>Disponível <strong>{selectedFlavorInventory?.available ?? current?.available_quantity ?? 0}</strong></span>
              {selectedFlavorInventory && <span>A caminho <strong>{selectedFlavorInventory.incoming}</strong></span>}
            </div>
          )}
        ...
        <div className="inventory-modal-actions">
          <button className="button ghost" type="button" onClick={close}>Cancelar</button>
          <button className="button gold" type="submit" disabled={loading}>
            {loading ? <LoaderCircle className="spin" size={16}/> : mode === "transfer" ? <ArrowRightLeft size={16}/> : mode === "count" ? <ClipboardCheck size={16}/> : <PackagePlus size={16}/>}
