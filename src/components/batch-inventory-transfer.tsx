...
import type { InventoryOverviewRow, LocationOption } from "@/lib/types";
...
export function BatchInventoryTransfer({ products, locations }: { products: InventoryOverviewRow[]; locations: LocationOption[] }) {
  ...
  return <form className="batch-transfer panel" onSubmit={submit}>
    ...
    <div className="batch-transfer-items">{items.map((item, index) => { const productFlavors = flavors.filter((row) => row.productId === item.productId); return <article key={index}><strong>Produto {index + 1}</strong><select required value={item.productId} onChange={(event) => change(index, { productId: event.target.value, flavorId: "" })}><option value="">Selecione o produto</option>{products.map((row) => <option key={row.product_id} value={row.product_id}>{row.product_name} · {row.available_quantity} disp.</option>)}</select>{productFlavors.length ? <select required value={item.flavorId} onChange={(event) => change(index, { flavorId: event.target.value })}><option value="">Selecione o sabor</option>{productFlavors.map((row) => <option key={row.id} value={row.id}>{row.name}</option>)}</select> : <span>Sem controle por sabor</span>}<input required type="number" min="1" value={item.quantity} onChange={(event) => change(index, { quantity: event.target.value })}/><button type="button" onClick={() => setItems((current) => current.filter((_, position) => position !== index))} disabled={items.length === 1} aria-label="Remover produto"><Trash2 size={17}/></button></article>; })}</div>
