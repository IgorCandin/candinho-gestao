...
import { Boxes, Building2, Layers3, MapPin, PackageSearch, Percent, Plus, Search, X } from "lucide-react";
...
export type CompanyPartnerStockRow = {
  ...
  imageUrl: string | null; salePrice: number; quantity: number; updatedAt: string | null;
};
...
export function CompanyProductsWorkspace({ products, combos, promotions, partnerStock, initialView = "all" }: { products: CompanyProductRow[]; combos: ProductComboRow[]; promotions: ActivePromotionRow[]; partnerStock: CompanyPartnerStockRow[]; initialView?: CatalogView }) {
  ...
  const visiblePartnerStock = useMemo(() => partnerStock.filter((row) => {
    const needle = query.trim().toLocaleLowerCase("pt-BR");
    return (partnerId === "all" || row.partnerId === partnerId) && (!needle || `${row.productName} ${row.brand ?? ""} ${row.category ?? ""} ${row.partnerName} ${row.locationName}`.toLocaleLowerCase("pt-BR").includes(needle));
  }), [partnerId, partnerStock, query]);
  const partnerUnits = visiblePartnerStock.reduce((sum, row) => sum + row.quantity, 0);
  const productMetric = view === "partners" ? new Set(visiblePartnerStock.map((row) => row.productId)).size : metricRows.length;
  const availableMetric = view === "partners" ? new Set(visiblePartnerStock.filter((row) => row.quantity > 0).map((row) => row.productId)).size : available.length;
  const unitMetric = view === "partners" ? partnerUnits : available.reduce((sum, product) => sum + product.available_quantity, 0);
  ...
  return <div className="company-workspace-v2">
    ...
    <section className="company-workspace-panel"><div className="company-workspace-toolbar"><div><button className={view === "all" ? "active" : ""} onClick={() => selectView("all")}>Todas</button><button className={view === "Suplementos" ? "active" : ""} onClick={() => selectView("Suplementos")}>Suplementos</button><button className={view === "Fitness" ? "active" : ""} onClick={() => selectView("Fitness")}>Fitness</button><button className={view === "combos" ? "active" : ""} onClick={() => selectView("combos")}>Combos</button><button className={view === "promotions" ? "active" : ""} onClick={() => selectView("promotions")}>Promoções</button><button className={view === "partners" ? "active" : ""} onClick={() => selectView("partners")}>Parceiros</button></div><label><Search size={17}/><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar produto, combo, promoção ou parceiro..."/></label></div>
      {view !== "combos" && view !== "promotions" && view !== "partners" ? <><div className="company-product-filters"><label><span>Disponibilidade</span><select value={availability} onChange={(event) => setAvailability(event.target.value as Availability)}><option value="all">Todas</option><option value="available">Com estoque</option><option value="incoming">Somente a caminho</option><option value="out">Zerados</option></select></label><label><span>Categoria</span><select value={category} onChange={(event) => setCategory(event.target.value)}><option value="all">Todas as categorias</option>{categories.map((item) => <option value={item} key={item}>{item}</option>)}</select></label><label><span>Marca</span><select value={brand} onChange={(event) => setBrand(event.target.value)}><option value="all">Todas as marcas</option>{brands.map((item) => <option value={item} key={item}>{item}</option>)}</select></label>{(availability !== "all" || category !== "all" || brand !== "all" || query) ? <button type="button" onClick={() => { setQuery(""); setAvailability("all"); setCategory("all"); setBrand("all"); }}>Limpar filtros</button> : null}</div><p className="company-workspace-count">{visible.length} produto(s)</p>
