...
import {
  consumeDeliveryFinalizationIntent,
...
export function SaleStatusActions({
  ...
  const materialsCost = deliverySupplies.reduce(
    (sum, item) =>
      sum + item.quantity * item.average_unit_cost,
    0,
  );

  async function beginDelivery(requestedDate?: string) {
    setMode("delivered");
    setMessage(null);

    if (requestedDate) {
      setDeliveredDate(requestedDate);
    }
    ...
    try {
      ...
      setDeliverySupplies(
        items.map((item) => ({
          ...
          unit_name: String(
            item.unit_name ?? "unidade",
          ),
          quantity_on_hand: numberValue(
            item.quantity_on_hand,
          ),
          average_unit_cost: numberValue(
            item.average_unit_cost,
          ),
          suggested_quantity: numberValue(
            item.suggested_quantity,
          ),
          suggestion_mode:
            item.suggestion_mode === "per_sale" ||
            item.suggestion_mode === "capacity"
      ...
      setMessage(
        getErrorMessage(
          error,
          "Não foi possível carregar os materiais da entrega.",
        ),
      );
    } finally {
  ...
  useEffect(() => {
    ...
    const requestedByUrl =
      url.searchParams.get("finalizar-entrega") === "1";

    const intent =
      consumeDeliveryFinalizationIntent();

    if (!requestedByUrl && !intent) return;
    ...
    url.searchParams.delete("finalizar-entrega");
    ...
    void beginDelivery(requestedDate);
  }, [canDeliver, saleId]);
    ...
    return (
      <div className="sale-actions-complete">
        ...
        <div>
          ...
          <span>
            Ela não entra nos indicadores comerciais nem
            mantém reserva de estoque.
          </span>
        </div>
      </div>
    );
      ...
      {canDeliver ? (
        ...
        <div className="sale-actions-complete">
          ...
          <div>
            ...
            <span>
              O pagamento é controlado separadamente no
              painel financeiro da venda.
            </span>
          </div>
        </div>
      )}
      ...
      {mode === "delivered" && (
        ...
        >
          <div className="sale-action-form-head">
            <div>
              <strong>Finalizar entrega</strong>
              <span>
                Confirme a data e a embalagem realmente
                usada. Este é o mesmo fechamento para venda,
                orçamento, lead e Agenda.
              </span>
            </div>
          ...
          <div className={deliveryStyles.materials}>
            <div className={deliveryStyles.intro}>
              <strong>
                Usou sacola/cartão nesta entrega?
              </strong>
              <span>
                Informe zero, uma ou várias unidades conforme
                o pedido real. Nada é descontado por
                suposição.
              </span>
            </div>
            ...
            {productUnits > 0 && (
              <div className={deliveryStyles.hint}>
                Esta venda possui{" "}
                <strong>{productUnits} produto(s)</strong>.
                A quantidade sugerida é apenas referência;
                confirme o que realmente saiu.
              </div>
            )}
            ...
            {loadingMaterials ? (
              ...
              <div className={deliveryStyles.hint}>
                Nenhuma sacola ou cartão está configurado
                para escolha na entrega. Você pode confirmar
                normalmente.
              </div>
            ) : (
              <div className={deliveryStyles.rows}>
                {deliverySupplies.map((item) => (
                  <div
                    className={deliveryStyles.row}
                    key={item.supply_id}
                  >
                    <div className={deliveryStyles.copy}>
                      <strong>{item.name}</strong>
                      <span>
                        Estoque {item.quantity_on_hand} ·{" "}
                        {formatCurrency(
                          item.average_unit_cost,
                        )}{" "}
                        por {item.unit_name}
                        {item.suggested_quantity > 0
                          ? ` · sugestão ${item.suggested_quantity}`
                          : ""}
                      </span>
                    </div>

                    <div
                      className={deliveryStyles.quantity}
                    >
                      <button
                        type="button"
                        onClick={() =>
            ...
            <div className={deliveryStyles.summary}>
              ...
              >
                <ShoppingBag size={13} /> Não usei
                sacola/cartão
              </button>

              <span>
                Custo desta embalagem:{" "}
                <strong>
          ...
          <div className="sale-action-form-head">
            <div>
              ...
              <span>
                ...
                automaticamente. Pagamentos já recebidos não
