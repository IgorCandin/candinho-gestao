...
function iconFor(scope: string) {
  ...
  if (scope === "company" || scope === "central" || scope === "marketing") {
    return Building2;
  }
  return Sparkles;
}
...
function QueueCard({ item, rank }: { item: NexusUnifiedQueueItem; rank: number }) {
  ...
  return (
    <Link className={`nexus-focus-priority-v455 ${item.severity}`} href={item.href}>
      ...
      <span className={`nexus-focus-op-v455 ${item.operation_scope}`}>
        <Icon size={14} />
      </span>
      <span className="nexus-focus-priority-copy-v455">
        <small>
          {item.operation_label}
          {item.due_at ? ` · ${dueLabel(item.due_at)}` : ""}
        </small>
  ...
  return (
    <article className={`nexus-personal-card-v455 ${shortcut.operation_scope}`}>
      <div className="nexus-personal-card-head-v455">
        <span className="nexus-personal-card-icon-v455">
          <Icon size={15} />
        </span>
      ...
      <Link
        href={shortcut.href}
        onClick={() => void recordUse(shortcut.id)}
      >
        <small>
          {operationLabel(shortcut.operation_scope)} ·{" "}
          {personalShortcutSourceLabel(shortcut.source)}
        </small>
...
export function NexusFocusWorkspace({
  ...
  const topQueue = useMemo(() => queue.items.slice(0, 5), [queue.items]);

  const reloadPersonal = useCallback(async () => {
    const response = await fetch("/api/nexus/personal?route=/nexus/foco", {
      cache: "no-store",
    });
  ...
  async function refreshAll() {
    ...
    try {
      ...
      if (personalResponse.ok) {
        setPersonal((await personalResponse.json()) as NexusPersonalWorkspace);
      }
      if (queueResponse.ok) {
        setQueue((await queueResponse.json()) as NexusUnifiedQueueSnapshot);
      }
    ...
    <section className="nexus-focus-workspace-v455">
      <div className="nexus-focus-hero-v455">
        <div>
          ...
          <p>
            Sem abrir cinco módulos para decidir o que fazer. Esta tela só
            organiza fontes oficiais e seus atalhos pessoais.
          </p>
        </div>
      ...
      <div className="nexus-focus-summary-v455">
        ...
        <article className="urgent">
          <span>Urgentes</span>
          <strong>{queue.summary.urgent}</strong>
          <small>merecem atenção primeiro</small>
        </article>
        <article>
          <span>Meus atalhos</span>
          <strong>{personal.stats.total_pins}</strong>
          <small>fixados por você</small>
        </article>
        <article>
          <span>Nexus sugere</span>
          <strong>{personal.stats.suggestion_count}</strong>
          <small>baseado em uso real</small>
        </article>
      </div>
      ...
      <article className="nexus-focus-panel-v455">
        <header>
          <div>
            ...
            <p>
              O Nexus só ordena. Ao abrir, você continua no módulo dono do dado.
            </p>
          </div>
          <Link className="button ghost compact-button" href="/nexus/fila">
            Ver Fila Única <ArrowRight size={13} />
          </Link>
        </header>
        ...
        <div className="nexus-focus-priorities-v455">
          {topQueue.length ? (
            topQueue.map((item, index) => (
              <QueueCard item={item} rank={index + 1} key={item.queue_id} />
            ))
          ) : (
            <div className="empty compact">
              <CheckCircle2 size={24} />
              <strong>Fila limpa.</strong>
              Nenhuma prioridade operacional apareceu agora.
            </div>
          )}
        </div>
      </article>
      ...
      <article className="nexus-focus-panel-v455">
        <header>
          <div>
            ...
            <p>
              Os quatro primeiros também funcionam por teclado: Alt+1 até Alt+4.
            </p>
          </div>
          <span className="nexus-focus-keyboard-v455">
            <Keyboard size={14} /> Alt + 1…4
          </span>
        </header>
        ...
        {personal.pinned.length ? (
          ...
          <div className="nexus-focus-onboarding-v455">
            <Pin size={18} />
            <div>
              <strong>Você ainda não fixou nenhum atalho.</strong>
              <span>
                Escolha uma sugestão abaixo. O ERP não muda sozinho: você decide
                quais caminhos merecem ficar permanentes.
              </span>
            </div>
          </div>
        )}
        ...
        <article className="nexus-focus-panel-v455">
          <header>
            <div>
              ...
              <h2><Sparkles size={17} /> O Nexus percebeu estes caminhos</h2>
              <p>
                Sugestões baseadas na sua navegação dos últimos 30 dias.
              </p>
            </div>
          </header>
          ...
          <div className="nexus-focus-suggestions-v455">
            {personal.suggested.map((item) => (
              <div className="nexus-focus-suggestion-v455" key={item.href}>
                <Link href={item.href}>
                  <small>
                    {operationLabel(item.operation_scope)} · {item.hits} uso(s) ·{" "}
                ...
                <NexusPinShortcutButton
                  href={item.href}
                  label={nexusRouteLabel(item.href)}
                  source="learned"
                  contextRoute="*"
                  onChanged={() => void reloadPersonal()}
                />
        ...
        <article className="nexus-focus-panel-v455 compact">
          <header>
            <div>
              ...
              <h2><Clock3 size={17} /> Continue de onde passou</h2>
            </div>
          </header>
          ...
          <div className="nexus-focus-recent-v455">
            {personal.recent.map((item) => (
              <Link href={item.href} key={item.href}>
                <strong>{nexusRouteLabel(item.href)}</strong>
                <small>{operationLabel(item.operation_scope)}</small>
              </Link>
            ))}
          </div>
        </article>
