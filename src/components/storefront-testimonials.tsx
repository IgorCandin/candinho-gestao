...
export function StorefrontTestimonials({ items }: { items: StorefrontTestimonial[] }) {
  return <section className="storefront-testimonials">
    ...
    {items.length ? <div className="storefront-testimonial-grid">{items.map((item) => <article key={item.id}><blockquote>“{item.comment}”</blockquote><footer><StorefrontProfileChip name={item.customer_name} profession={item.profession} photoUrl={item.photo_url}/></footer></article>)}</div> : <div className="storefront-testimonial-empty">Os primeiros depoimentos reais aparecerão aqui em breve.</div>}
  </section>;
}
import { StorefrontProfileChip } from "@/components/storefront-profile-chip";
