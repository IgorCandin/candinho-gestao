...
export function LeadsTable({ leads }: { leads: LeadRow[] }) {
  ...
  return <div className="table-wrap"><table className="leads-table"><thead><tr>
    ...
    {sortedLeads.map((lead) => <tr key={`${lead.id}:${lead.item_id ?? lead.primary_product_id ?? "lead"}`}>
      ...
      <td>{lead.primary_product_id ? <Link className="table-link" href={`/produtos/${lead.primary_product_id}`}>{lead.product_summary ?? "Produto"}</Link> : lead.product_summary ?? "—"}</td>
      ...
      <td><Link className="icon-link" href={`/leads/${lead.id}`} aria-label={`Abrir lead de ${lead.customer_name}`}><ArrowRight size={18} /></Link></td>
    </tr>)}
  </tbody></table></div>;
}
