...
begin
  ...
  if p_customer_id is null or p_due_at is null then
    raise exception 'Cliente e data são obrigatórios';
  end if;

  if coalesce(p_priority,'normal') not in ('normal','attention','urgent') then
  ...
  where ot.customer_id=p_customer_id
    and ot.category='follow_up'
    and ot.status='planned'
    and ot.cancelled_at is null
    and ot.completed_at is null
    and coalesce(ot.notes,'') like '[Radar]%'
  ...
  if v_existing is not null then
    update public.operational_tasks
      set due_at=p_due_at,
          ...
          notes='[Radar] '||coalesce(
            nullif(btrim(p_notes),''),
            'Retorno sugerido pelo Radar de oportunidades.'
          ),
