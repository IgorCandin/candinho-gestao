  ...
  if v_existing is not null then
    update public.operational_tasks
      set due_at=p_due_at,
          ...
          notes='[Radar] '||coalesce(nullif(btrim(p_notes),''),'Retorno sugerido pelo Radar de oportunidades.'),
    ...
    insert into public.operational_tasks(
      ...
      v_contact_id,
      auth.uid(),
      '[Radar] '||coalesce(nullif(btrim(p_notes),''),'Retorno sugerido pelo Radar de oportunidades.'),
      auth.uid()
    ) returning id into v_task_id;
  end if;
