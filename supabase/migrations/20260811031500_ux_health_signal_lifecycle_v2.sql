...
alter table public.ux_health_signals
...
begin
  alter table public.ux_health_signals
    add constraint ux_health_signals_status_check
...
create index if not exists ux_health_signals_user_status_recent_idx
  on public.ux_health_signals(user_id,status,last_seen_at desc);

-- Tudo que já existia antes desta correção vira histórico.
-- Se o mesmo problema reaparecer depois, a RPC abaixo reabre
...
set
  ...
  resolution_note = coalesce(
    resolution_note,
    'Baseline V45.20: sinal anterior à correção. Reabre automaticamente se reaparecer.'
  )
where status = 'active';
...
begin
  ...
  on conflict(user_id,fingerprint) do update
    set
      ...
      overflow_px = excluded.overflow_px,
