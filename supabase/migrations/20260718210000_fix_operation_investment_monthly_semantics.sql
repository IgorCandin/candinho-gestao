-- Candinho Company · V11
-- Corrige a semântica de "investimento mensal":
-- investimento do mês = custo dos pedidos feitos no mês.
-- Não soma "recebido no mês + pendente", pois isso mistura pedidos de meses diferentes.

create or replace function public.bank_operation_investment_snapshot(p_reference_month date default null::date)
returns jsonb
language plpgsql
stable
security definer
...
begin
  ...
  return jsonb_build_object(
    ...
    'supplements',jsonb_build_object(
      ...
      'monthly_invested',v_supp_month_ordered,
    ...
    'fitness',jsonb_build_object(
      ...
      'monthly_invested',v_fit_month_ordered,
    ...
    'company',jsonb_build_object(
      'monthly_invested',v_supp_month_ordered+v_fit_month_ordered,
