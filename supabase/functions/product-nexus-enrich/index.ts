...
async function requestModel(apiKey: string, model: string, prompt: string, useWeb: boolean) {
  ...
  let lastError = "Pesquisa indisponível";
  for (let attempt = 0; attempt < 2; attempt += 1) {
    ...
    lastError = typeof apiError?.message === "string" ? apiError.message : `Pesquisa falhou (${response.status})`;
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return json({ error: error instanceof Error ? error.message : "Falha ao pesquisar produto" }, 500);
  }
});
